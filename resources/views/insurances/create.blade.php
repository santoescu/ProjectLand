<x-layouts.app :title="__('Insurance')">

    @include('partials.tittle', [
    'title' => __("Create :name", ['name' => __('Insurance')]) ,
    'subheading' => ""
    ])

    <form method="POST" action="{{ route('insurances.store') }}" class="space-y-4 max-w-xl mx-auto">
        @csrf
        ...
        @include('insurances.partials.date-range-picker', [
            'fieldPrefix' => 'create_insurance',
            'initialEffective' => old('effective_date'),
            'initialExpiration' => old('expiration_date'),
        ])

        <flux:input label="{{__('Link')}}" name="link" :value="old('link')" />
