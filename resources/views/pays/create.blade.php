<x-layouts.app :title="__('Payable')">
    ...
    <form id="createPayForm" method="POST" action="{{ route('pays.store') }}" class="space-y-4 max-w-xl mx-auto">
        @csrf
        ...
        <div x-data="contractSelect({
                ...
                oldContractorId: @js(old('contractor_id')),
                oldContractId: @js(old('contract_id'))
                })"
             x-init="init()">
            <div data-flux-field class="relative {{ $errors->has('contractor_id') ? 'error' : '' }}">
                ...
                <select data-hs-select='{
                      ...
                      }' id="contractor_id" name="contractor_id" :value="old('contractor_id')" @change="onContractorChange($event.target.value)">
                    <option value=""></option>
        ...
        <div class="flex justify-end gap-2">
            <flux:button type="button" variant="primary" onclick="openCreatePayModal()">{{__('Save')}}</flux:button>
    ...
    <div id="confirm-create-pay-modal"
         ...
         role="dialog"
         tabindex="-1"
         aria-labelledby="confirm-create-pay-label"
         wire:ignore>
        ...
        <div class="hs-overlay-open:mt-7 hs-overlay-open:opacity-100 hs-overlay-open:duration-500 mt-0 opacity-0 ease-out transition-all sm:max-w-lg sm:w-full m-3 sm:mx-auto min-h-[calc(100%-56px)] flex items-center">
            <div class="w-full flex flex-col bg-white border border-gray-200 shadow-2xs rounded-xl pointer-events-auto dark:bg-neutral-800 dark:border-neutral-700 dark:shadow-neutral-700/70">
                <div class="flex justify-between items-center py-3 px-4 border-b border-gray-200 dark:border-neutral-700">
                    <h3 id="confirm-create-pay-label" class="font-bold text-gray-800 dark:text-white">
                        {{ __("Create :name?", ['name' => __('Pay')]) }}
                    </h3>
                    ...
                    <button type="button"
                            ...
                            aria-label="Close"
                            data-hs-overlay="#confirm-create-pay-modal">
                ...
                <div class="px-6 py-8 sm:px-8 sm:py-10 text-center">
                    ...
                    <p class="text-base sm:text-lg font-medium text-gray-800 dark:text-white">
                        {{ __("Do you want to confirm the creation of this :name?", ['name' => __('Pay')]) }}
                    </p>

                    <p class="mt-2 text-sm sm:text-base text-gray-500 dark:text-neutral-300">
                        {{ __('The payment request will be created after confirmation.') }}
                    </p>
                </div>
    ...
    <script>
        ...
        function openCreatePayModal() {
            if (window.HSOverlay) {
                HSOverlay.autoInit();
                HSOverlay.open('#confirm-create-pay-modal');
            }
        }
        ...
        function submitCreatePayForm() {
            ...
            if (amountInput) {
                amountInput.value = normalizePayMoneyInput(amountInput.value);
            }
